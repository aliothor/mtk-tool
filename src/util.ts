 ...
 * @returns
 */
export async function runCommand(inputDir: string = testInputDir, outputDir: string = testOutputDir) {
 ...
 * @returns
 */
async function readAllWorkerSourceFileName(sourcePath: string) {
 ...
 * @param outputDir
 */
async function buildWorkerToAmdFormat(inputDir: string, outputDir: string) {
  /**
   * rollup构建用到的plugin数组
   */
  const pluginArray = [
 ...
 * @param inputDir
 */
async function rewriteAllTempFile(inputDir: string, outputDir: string) {
 ...
 * @param outputDir
 */
async function rewriteWorkerFile(filepath: string, outputDir: string) {
